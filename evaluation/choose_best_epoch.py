...
for i, sigma in enumerate(np.arange(0.1, 1.1, 0.1)):
    # change this path if you use different structure for your directories inside the experiment
    path = exp_path.joinpath(f'{dataset}/sigma{sigma:.1f}')
    for split in range(0, 5):
        results_file = path.joinpath(f'results/split{split}/f_scores.txt')
        logs_file = path.joinpath(f'logs/split{split}/scalars.csv')
